...
class OSButtons extends React.Component<Props, State> {
  ...
  createDeviceFields() {
    ...
    const deviceStateButton = renderButtonView(
      'Get Device State',
      color,
      async () => {
    ...
    const requireUserProvideConsent = renderButtonView(
      this.state.requireUserConsent
        ? 'Remove User Privacy Consent Requirement'
        : 'Require User Privacy Consent',
      color,
      () => {
        loggingFunction(
          `Require User Consent: ${!this.state.requireUserConsent}`,
        );
    ...
    const provideUserConsentButton = renderButtonView(
      this.state.provideUserConsent
        ? 'Reject User Consent'
        : 'Provide User Consent',
      color,
      async () => {
        loggingFunction(
          `Provide User Consent: ${!this.state.provideUserConsent}`,
        );
    ...
    const userProvidedPrivacyConsent = renderButtonView(
      'Did User Provide Privacy Consent',
      color,
      async () => {
  ...
  createNotificationFields() {
    ...
    const deleteTagWithKey = renderButtonView(
      'Delete Tag With Key',
      color,
      async () => {
  ...
  createEmailFields() {
    ...
    const externalUserIdButton = renderButtonView(
      ...
      () => {
        ...
        OneSignal.setExternalUserId(
          ...
          (res: object) => {
            loggingFunction(
              'setExternalUserId completed with result: ',
              JSON.stringify(res),
            );
          },
        );
    ...
    const removeExternalIdButton = renderButtonView(
      ...
      () => {
        ...
        OneSignal.removeExternalUserId((res: object) => {
          loggingFunction(
            'removeExternalUserId completed with result: ',
            JSON.stringify(res),
          );
        });
      },
    ...
    elements.push(
      setEmailButton,
      logoutEmailButton,
      externalUserIdButton,
      removeExternalIdButton,
    );
    return elements;
  }
